...
(function () {
    ...
    // Barramento cross-tab e local (suporta múltiplos listeners e evita eco)
    const bus = (() => {
        ...
        const addListener = (topic, fn) => {
            if (!listeners.has(topic)) listeners.set(topic, new Set());
            listeners.get(topic).add(fn);
        };
        const dispatch = (topic, payload) => {
            const set = listeners.get(topic);
            if (!set) return;
            for (const fn of set) {
                try { fn(payload); }
                catch (err) { console.error(`[ps:bus] handler error for "${topic}"`, err); }
            }
        };
        ...
        if (bc) {
            bc.addEventListener("message", (ev) => {
                ...
                if (msg.sender && msg.sender === senderId) return; // evita eco
                dispatch(msg.t, msg.payload);
            });
        }

        const emit = (t, payload) => {
            if (bc) bc.postMessage({ t, payload, sender: senderId });
    ...
    const store = (() => {
        ...
        function createProject(nome) {
            ...
            let createdId = 1;
            patch((s) => {
                const ids = (s.projetos || []).map((p) => Number(p.Id) || 0);
