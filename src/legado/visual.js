...
/*
  ...
  - $ / $$: seletores DOM rápidos.
  - Constantes: tipos/prioridades/status + opções e PLACEHOLDER_IMG.
  - escapeHtml / formatarData / formatarDataInput / imgSrc / mediaDisp: helpers de formatação e imagens.
  - showToast / runSectionEnter: UI de toast e animação de entrada de cards.
  - initDashboardCharts / updateLineChart / updateDashboardCharts / renderDashboardData: gráficos e KPIs do dashboard.
  - renderSetup / renderDescricaoEvento / renderDispositivos / renderCronograma / renderKanban / renderSimulacao / renderRecurso: renderizadores principais das páginas (recebem state).
  - updateLogoPreview / mostrarDetalheDispositivoFull / mostrarDetalheSim: modais e visuais auxiliares.
  - createKanbanCard / showMoveMenu / setupKanbanDropZones: cartões Kanban, menu mobile de mover e DnD.
*/
...
export function updateDashboardCharts(state) {
    ...
    dispositivos.forEach((d) => {
        const st = Object.prototype.hasOwnProperty.call(counts, d.StatusManual) ? d.StatusManual : "NoPrazo";
...
export function renderSetup(state) {
    ...
    // Carrega os dados do projeto nos campos (usa novos IDs do aplicacao.jsx)
    if (proj) {
        const info = proj.Info || {};
        const setVal = (id, val) => { const el = $(id); if(el) el.value = val || ""; };
        setVal("#setup-centro-custo", info.CentroCusto || "");
        setVal("#setup-cliente", info.Cliente || proj.Cliente || "");
        setVal("#setup-planta", info.Planta || "");
        setVal("#setup-projeto", info.ProjetoNome || proj.Nome || "");
        setVal("#setup-area", info.Area || "");
        setVal("#setup-design-leader", info.DesignLeader || "");
        setVal("#setup-technical-leader", info.TechnicalLeader || "");
    }

    // Renderiza a tabela de milestones
    const tb = $("#setup-milestones-table");
    if (tb) {
        tb.innerHTML = "";
        const milestones = (state.classes || []).filter((c) => c.ProjetoId === pid);

        if(milestones.length === 0) {
            tb.innerHTML = '<tr><td colspan="5" style="text-align:center; padding:40px; color:#94a3b8;">Nenhum milestone definido.</td></tr>';
        } else {
            milestones.forEach((c) => {
                const tr = document.createElement("tr");
                tr.innerHTML = `
                    <td><div style="width:20px; height:20px; border-radius:6px; background:${c.CorHex || "#cbd5e1"};"></div></td>
                    <td><strong style="color:#1e293b;">${escapeHtml(c.Nome)}</strong></td>
                    <td><span class="badge ${c.Percentual >= 100 ? 'ok' : ''}" style="padding:4px 10px; font-size:12px;">${c.Percentual || 0}%</span></td>
                    <td>${formatarData(c.DataBase)}</td>
                    <td style="text-align:right;">
                        <button class="btn btn-sm ghost" data-action="edit-classe" data-id="${c.Id}" style="padding:6px 10px;">
                            Editar
                        </button>
                        <button class="btn btn-sm ghost" data-action="delete-classe" data-id="${c.Id}" style="padding:6px 10px; color:#ef4444;">
                            Excluir
                        </button>
                    </td>
                `;
