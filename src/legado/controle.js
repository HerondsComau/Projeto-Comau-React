...
import * as View from './visual.js';

/*
  Guia rápido (comentários apenas):
  - syncStateFromStore / persistState / tryLoadSaved / carregarEstadoInicial: sincronizam o estado global com o store/localStorage e reconstroem datas.
  - classesDoProjeto / dispositivosDoProjeto: seletores utilitários por projeto.
  - initCorporateSplash: mostra e remove a splash corporativa.
  - clearAllData: limpa storage e reinicia estado.
  - calcularStatusPrazo / snapshotNow / recordSnapshot / persistMetrics: calculam status de prazo e snapshots de progresso.
  - gerarDadosDemo: popula dados de exemplo.
  - reviveDates: converte strings de data para Date.
  - novoProjeto / verificarProjetoExistente: criação de projeto sincronizada e verificação inicial.
  - abrirModalDispositivo / salvarModalDispositivo / fecharModalDispositivo e auxiliares: CRUD do modal de dispositivos.
  - setupFormClasse* / setupFormMilestone*: CRUD de milestones/classes.
  - render* wrappers: chamam as versões em views.js passando o state.
  - showProjetoMecanicoPage: router interno das abas do status com animação e cache de render.
  - atualizarProjetoSelector / setProjetoAtual: dropdown custom de projetos.
  - bindGlobalEvents: listeners globais (cliques, filtros, modais, broadcast).
  - enhanceNav: move a inkbar da navegação.
  - ViewRouter init + event hooks no DOMContentLoaded: boot geral do app.
  - toggleDrawer / mobileNavigate: controle do drawer mobile.
*/
...
const ENABLE_EXPANDO = true;
let currentProjetoPage = "dashboard";
...
// ===== Splash corporativa (fail-safe / Elastic Pop) =====
function removerSplashOverlayAgora() {
    const overlay = document.getElementById("splash-overlay");
    if (!overlay) return;
    overlay.classList.add("splash-hide");
    window.setTimeout(() => { try { overlay.remove(); } catch { } }, 520);
}
window.removerSplashOverlayAgora = removerSplashOverlayAgora;
...
function initCorporateSplash() {
    ...
    // Fail-safe extra: se ninguém chamar done, sai em ~4.5s
    const kill = setTimeout(() => { if (!finished) { set(1, "Continuando…"); done(); } }, 4500);

    return {
        set,
        done: () => { clearTimeout(kill); done(); },
        removeNow: () => { clearTimeout(kill); removerSplashOverlayAgora(); }
    };
}
window.initCorporateSplash = window.initCorporateSplash || initCorporateSplash;
...
function novoProjeto(opts = {}) {
    ...
    if (!skipNavigate) {
        ...
        setTimeout(() => {
            showProjetoMecanicoPage("dashboard", { force: true });
...
const TAB_ORDER = ["descricaoevento", "dispositivos", "cronograma", "dashboard", "kanban", "recurso"];
...
function abrirTelaSetupProjeto() {
    try {
        renderSetup();
        atualizarProjetoSelector();
    } catch { }
    if (window.__setupModoCriacao) {
        try { novoProjeto({ skipNavigate: true }); } catch { }
        window.__setupModoCriacao = false;
    }
    setTimeout(() => {
        const inputNome = document.getElementById("pd-projeto");
        if (inputNome) inputNome.focus();
...
function rebindSetupEvents() {
    // Suporte para IDs antigos e novos
    $("#btn-save-project-data")?.addEventListener("click", salvarDadosProjeto);
    $("#btn-salvar-setup-mecanica")?.addEventListener("click", salvarDadosProjeto);
    ...
    $("#btn-novo-milestone")?.addEventListener("click", () => {
        // TODO: Implementar modal de criação de milestone
        View.showToast("Funcionalidade de milestone em desenvolvimento", "info");
    });
    $("#setup-classe-cancelar")?.addEventListener("click", () => {
        $("#setup-form-container")?.classList.remove("open");
    });
    $("#setup-classe-salvar")?.addEventListener("click", setupFormClasseSalvar);
    $("#btn-setup-concluir")?.addEventListener("click", () => {
        salvarDadosProjeto();
        window.__statusIntentTab = "dashboard";
        location.hash = "#status";
    });
    $("#btn-setup-cancelar")?.addEventListener("click", () => {
...
function salvarDadosProjeto() {
    ...
    // Suporta tanto IDs antigos quanto novos
    const nomeInput = document.getElementById("setup-projeto") || document.getElementById("pd-projeto");
    const novoNome = nomeInput?.value?.trim() || `Projeto ${pid}`;

    const info = {
        CentroCusto: (document.getElementById("setup-centro-custo") || document.getElementById("pd-cc"))?.value,
        Cliente: (document.getElementById("setup-cliente") || document.getElementById("pd-cliente"))?.value,
        Planta: (document.getElementById("setup-planta") || document.getElementById("pd-planta"))?.value,
        ProjetoNome: novoNome,
        Area: (document.getElementById("setup-area") || document.getElementById("pd-area"))?.value,
        DesignLeader: (document.getElementById("setup-design-leader") || document.getElementById("pd-dl"))?.value,
        TechnicalLeader: (document.getElementById("setup-technical-leader") || document.getElementById("pd-tl"))?.value,
    ...
    persistState(true);
    atualizarProjetoSelector();
    View.showToast("Configurações salvas com sucesso!", "ok");
}

function showProjetoMecanicoPage(target, opts = {}) {
    const { animate = true, force = false } = opts;
    if (!target) return;
    if (target === "setup") target = "dashboard";
...
function setProjetoAtual(id) {
    ...
    atualizarProjetoSelector();
    renderTodasPaginas();
    if (document.querySelector(".cs-pages .page")) showProjetoMecanicoPage("dashboard");
...
function bindGlobalEvents() {
    ...
    $("#btn-salvar-setup-mecanica") && $("#btn-salvar-setup-mecanica").addEventListener("click", salvarDadosProjeto);
    ...
    $("#btn-novo-milestone") && $("#btn-novo-milestone").addEventListener("click", () => {
        View.showToast("Funcionalidade de milestone em desenvolvimento", "info");
    });
    ...
    // REMOVIDO: O botão "+" agora é gerenciado pelo React (modal Setup Inicial)
    // O event listener está em aplicacao.jsx que abre o modal correto
    // const btnNovo = document.getElementById("btn-abrir-setup") || document.getElementById("btn-novo-projeto");
    // if (btnNovo) {
    //     btnNovo.addEventListener("click", (e) => {
    //         e.preventDefault();
    //         window.__setupModoCriacao = true;
    //         location.hash = "#setup";
    //     });
    // }
    ...
    if (bc) {
        bc.addEventListener("message", (ev) => {
            const msg = ev?.data || {};
            const t = msg.t;
            const mySender = window.PS?.bus?.senderId;
            if (mySender && msg.sender && msg.sender === mySender) return;
...
const ViewRouter = (() => {
    const TRANS_MS = 500;
    const VALID = new Set(["home", "setup", "status", "simulacao"]);
    let stack = ["home"];
    let busy = false;
    let pendingExpando = null;
    const EXPANDO_MS = 620;
    ...
    function findHomeCardFor(viewName) {
        if (viewName === "status" && window.__statusLastTabFromHome) {
            const sel = `.hub-card[data-route="status"][data-status-tab="${window.__statusLastTabFromHome}"]`;
    ...
    function setHeader(view) {
        ...
        if (title) {
            title.textContent = view === "home" ? "Central de Controle" : view === "setup" ? "Setup do Projeto" : view === "status" ? "Projetos Mecanicos" : "Gestao de Simulacao";
        }
        if (sub) {
            sub.textContent = view === "home" ? "HTML/JS Preview - Comau" : view === "setup" ? "Configuração geral do projeto" : view === "status" ? "Controle de Status - Dispositivos/Dashboard" : "Kanban de Liberacao - Process Simulate";
        }
    }
    ...
    function afterEnter(view) {
        if (view === "home") {
            ...
            abrirTelaSetupProjeto?.();
        } else if (view === "status") {
            enhanceNav?.();
            renderTodasPaginas?.();
            const pid = getProjetoIdAtual?.() || 1;
            let last = PS?.utils?.getLastTab ? PS.utils.getLastTab(pid) : currentProjetoPage;
            if (last === "setup") last = "dashboard";
            const intent = window.__statusIntentTab;
            const targetTab = intent || last || "dashboard";
            if (intent) window.__statusIntentTab = null;
            showProjetoMecanicoPage?.(targetTab, { animate: false, force: true });
    ...
    function apply(cur, next, dir, animate) {
        ...
        const canExpandoForward =
            ...
            cur === "home" &&
            (next === "status" || next === "simulacao" || next === "setup") &&
        ...
        const canExpandoBack =
            ...
            next === "home" &&
            (cur === "status" || cur === "simulacao" || cur === "setup");
    ...
    function onHashChange(first = false) {
        ...
        if (!pendingExpando && isBack && next === "home" && (cur === "status" || cur === "simulacao" || cur === "setup")) {
    ...
    function bind() {
        document.body.addEventListener("click", (e) => {
            ...
            if (target === "status") {
                const intentTab = a.dataset.statusTab;
                if (intentTab) window.__statusIntentTab = intentTab;
                if (isHubCard && intentTab) window.__statusLastTabFromHome = intentTab;
            }
        ...
        backBtn?.addEventListener("click", () => {
            ...
            const reduced = window.matchMedia && window.matchMedia("(prefers-reduced-motion: reduce)").matches;
            if (!reduced && prev === "home" && (cur === "status" || cur === "simulacao" || cur === "setup")) {
                pendingExpando = { type: "back", toEl: findHomeCardFor(cur) };
            }
            location.hash = "#" + prev;
        });
    ...
    function init() {
        ...
        console.log("[ROUTER] hash:", location.hash);
        console.log("[ROUTER] initial view:", initial);
        stack = ["home"];
        if (initial !== "home") stack.push(initial);

        // IMPORTANTE: Não remove .view-active se a view inicial é home e ela já está ativa
        // Isso evita interferir com os cards React que já foram renderizados
        const homeEl = el("home");
        const alreadyActiveHome = homeEl?.classList.contains("view-active") && initial === "home";

        if (!alreadyActiveHome) {
            ["home", "status", "simulacao", "setup"].forEach((v) => el(v)?.classList.remove("view-active"));
        }

        let initialEl = el(initial);
        if (!initialEl) {
            console.warn("[ROUTER] view not found, forcing home");
            location.hash = "#home";
            initialEl = el("home");
        }

        // Só adiciona .view-active se não for home já ativa (evita flicker)
        if (!alreadyActiveHome) {
            initialEl?.classList.add("view-active");
        }

        if (document.querySelectorAll(".view.view-active").length === 0) {
            const home = document.getElementById("view-home");
            if (home) {
                home.classList.add("view-active");
                ["view-exit-left", "view-exit-right", "view-enter-left"].forEach((cls) => home.classList.remove(cls));
                console.log("[ROUTER] Forçando view-home.view-active (fallback)");
            }
        }
        setHeader(initial);
        afterEnter(initial);
        if (!location.hash) location.hash = "#home";
        console.table([...document.querySelectorAll(".view")].map((v) => ({
            id: v.id,
            active: v.classList.contains("view-active"),
            opacity: getComputedStyle(v).opacity,
            transform: getComputedStyle(v).transform,
            display: getComputedStyle(v).display
        })));
        setTimeout(() => {
            console.log("[DEBUG] active views:", [...document.querySelectorAll(".view.view-active")].map((v) => v.id));
            console.log("[DEBUG] view-home rect:", document.getElementById("view-home")?.getBoundingClientRect());
            console.log("[DEBUG] home-cards-react rect:", document.getElementById("home-cards-react")?.getBoundingClientRect());
            console.log("[DEBUG] ios-home exists?", !!document.querySelector(".ios-home"));
            console.log("[DEBUG] ios-card-wrap count:", document.querySelectorAll(".ios-card-wrap").length);
        }, 500);
        bind();
    }
...
export async function iniciarControleLegado() {
    if (window.__controleLegadoIniciado) return;
    window.__controleLegadoIniciado = true;

    const failsafeTimer = window.setTimeout(() => {
        console.warn("[Splash] failsafe acionado: removendo overlay");
        removerSplashOverlayAgora();
    }, 4500);
    ...
    try {
        ...
        try { splash = initCorporateSplash(); } catch { }
        try { splash?.set(0.15, "Carregando interface…"); } catch { }
        ...
        console.error("[ControleLegado] Falha na inicialização:", err);
        try { splash?.set?.(1, "Erro — clique para continuar"); } catch { }
        window.__controleLegadoIniciado = false;
        setTimeout(() => { try { splash?.removeNow?.(); } catch { } }, 900);
    } finally {
        window.clearTimeout(failsafeTimer);
        try { splash?.done?.(); } catch { removerSplashOverlayAgora(); }
        removerSplashOverlayAgora();
    }
}
